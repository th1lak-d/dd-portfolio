    ...
    <section className="border-b border-line">
      <div className="max-w-5xl mx-auto px-6 md:px-10 py-24">
        <Reveal className="flex flex-wrap items-baseline justify-between gap-4 mb-12">
          ...
          <div className="font-mono text-xs px-3 py-1.5 border border-active/40 text-active rounded-full bg-active/5">
            ...
            <Reveal key={g.label}>
              ...
              <ul className="space-y-2">
                {g.items.map((item) => (
                  <li
                    key={item}
                    className="text-ink text-sm border-l-2 border-line pl-3 hover:border-accent transition-colors"
                  >
                    {item}
                  </li>
                ))}
